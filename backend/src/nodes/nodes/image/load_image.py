...
@NodeFactory.register("machines:image:load")
class ImReadNode(NodeBase):
    def __init__(self):
        super().__init__()
        self.description = "Load chart from specified file."
        self.inputs = [ImageFileInput(primary_input=True)]
        self.outputs = [
            LargeImageOutput(),
            DirectoryOutput("Chart Directory", of_input=0),
            FileNameOutput("Chart Name", of_input=0),
        ]
    ...
    def run(self, path: str) -> Tuple[np.ndarray, str, str]:
        ...
        if img is None:
            if error is not None:
                raise error
            raise RuntimeError(
                f'The image "{path}" you are trying to read cannot be read by MachinesStudio.'
            )
