...
export const parseFunctionDefinitions = (
    ...
    const parentScope = getMachinesStudioScope();
                ...
                try {
                    ...
                    for (const d of definitions) {
                        if (parentScope.has(d.name)) {
                            errors.push(
                                `Duplicate type definitions for ${d.name} in ${schema.schemaId} > ${input.label} (id: ${input.id}). The type definition is already defined in machines scope (see "src/common/types/chainner-scope.ts")`
                            );
                            continue;
                        }
