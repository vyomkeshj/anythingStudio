...
const toV080: ModernMigration = (data) => {
    data.nodes.forEach((node) => {
        ...
        if (node.data.schemaId === 'machines:image:resize_factor') {
            node.data.inputData[1] = (node.data.inputData[1] as number) * 100.0;
        }
        if (node.data.schemaId === 'machines:image:average_color_fix') {
            node.data.inputData[2] = (node.data.inputData[2] as number) * 100.0;
        }
        // Invert interpolation weight
        if (
            ['machines:pytorch:interpolate_models', 'machines:ncnn:interpolate_models'].includes(
                node.data.schemaId
            )
        ) {
...
const updateAdjustmentScale: ModernMigration = (data) => {
    data.nodes.forEach((node) => {
        ...
        if (node.data.schemaId === 'machines:image:hue_and_saturation') {
            node.data.inputData[2] = (((node.data.inputData[2] as number) / 255) * 100.0).toFixed(
                1
            );
        }
        if (node.data.schemaId === 'machines:image:brightness_and_contrast') {
...
const fixBlurNode: ModernMigration = (data) => {
    data.nodes.forEach((node) => {
        ...
        if (node.data.schemaId === 'machines:image:blur') {
            node.data.schemaId = 'machines:image:gaussian_blur' as SchemaId;
...
const addBlendNode: ModernMigration = (data) => {
    data.nodes.forEach((node) => {
        ...
        if (node.data.schemaId === 'machines:image:difference') {
            node.data.schemaId = 'machines:image:blend' as SchemaId;
        ...
        if (node.data.schemaId === 'machines:image:overlay') {
            ...
            if (edgesToChange.input !== undefined) {
                ...
                const newBlendNode: N = {
                    data: {
                        schemaId: 'machines:image:blend' as SchemaId,
...
const updateRotateNode: ModernMigration = (data) => {
    data.nodes.forEach((node) => {
        ...
        if (node.data.schemaId === 'machines:image:rotate') {
...
const addOpacityNode: ModernMigration = (data) => {
    const createOpacityNode = (node: N, opacityValue: InputValue, yMoveDirection: number) => {
        ...
        const newNode: N = {
            data: {
                schemaId: 'machines:image:opacity' as SchemaId,
    ...
    data.nodes.forEach((node) => {
        if (node.data.schemaId === 'machines:image:blend') {
            const findEdgesToChange = () => {
...
const fixDropDownNumberValues: ModernMigration = (data) => {
    const needToConvert = new Set([
        'machines:image:resize_factor/2',
        'machines:image:resize_resolution/3',
        'machines:image:threshold/3',
        'machines:image:threshold_adaptive/2',
        'machines:image:threshold_adaptive/3',
        'machines:image:color_transfer/3',
        'machines:image:color_transfer/4',
        'machines:image:blend/2',
        'machines:image:change_colorspace/1',
        'machines:image:create_border/1',
        'machines:image:rotate/2',
        'machines:image:rotate/3',
        'machines:image:rotate/4',
        'machines:image:flip/1',
        'machines:image:fill_alpha/1',
    ]);
...
const onnxConvertUpdate: ModernMigration = (data) => {
    const createOnnxSaveNode = (node: N, directory: InputValue, modelName: InputValue) => {
        ...
        const newNode: N = {
            data: {
                schemaId: 'machines:onnx:save_model' as SchemaId,
    ...
    data.nodes.forEach((node) => {
        if (node.data.schemaId === 'machines:pytorch:convert_to_onnx') {
            ...
            downstreamNodes.forEach((downstreamNode) => {
                if (downstreamNode.data.schemaId === 'machines:onnx:load_model') {
...
const blockSizeToRadius: ModernMigration = (data) => {
    data.nodes.forEach((node) => {
        if (node.data.schemaId === 'machines:image:threshold_adaptive') {
...
const removeTargetTileSize: ModernMigration = (data) => {
    data.nodes.forEach((node) => {
        if (
            ['machines:ncnn:upscale_image', 'machines:onnx:upscale_image'].includes(
                node.data.schemaId
            )
        ) {
...
const addTargetTileSizeAgain: ModernMigration = (data) => {
    data.nodes.forEach((node) => {
        if (
            [
                'machines:ncnn:upscale_image',
                'machines:onnx:upscale_image',
                'machines:pytorch:upscale_image',
    ...
    data.nodes.forEach((node) => {
        if (node.data.schemaId === 'machines:image:brightness_and_contrast') {
            ...
            if (brightness !== 0) {
                ...
                newNodes.push({
                    data: {
                        schemaId: 'machines:image:hue_and_saturation' as SchemaId,
...
const convertColorSpaceFromTo: ModernMigration = (data) => {
    ...
    data.nodes.forEach((node) => {
        if (node.data.schemaId === 'machines:image:change_colorspace') {
...
const convertNormalGenerator: ModernMigration = (data) => {
    data.nodes.forEach((node) => {
        if (node.data.schemaId === 'machines:image:normal_generator') {
...
const fixNumbers: ModernMigration = (data) => {
    // https://github.com/MachinesStudio-org/MachinesStudio/issues/1364

    const numbers: Record<string, number[] | undefined> = {
        'machines:image:save': [5],
        'machines:image:spritesheet_iterator': [1, 2],
        'machines:image:simple_video_frame_iterator_save': [5],
        'machines:image:crop_border': [1],
        'machines:image:crop_content': [1],
        'machines:image:crop_edges': [1, 2, 3, 4],
        'machines:image:crop_offsets': [1, 2, 3, 4],
        'machines:image:resize_factor': [1],
        'machines:image:resize_resolution': [1, 2],
        'machines:image:resize_to_side': [1],
        'machines:image:tile_fill': [1, 2],
        'machines:image:brightness_and_contrast': [1, 2],
        'machines:image:gamma': [1],
        'machines:image:hue_and_saturation': [1, 2, 3],
        'machines:image:opacity': [1],
        'machines:image:threshold': [1, 2],
        'machines:image:threshold_adaptive': [1, 4, 5],
        'machines:image:add_noise': [3],
        'machines:image:average_color_fix': [2],
        'machines:image:bilateral_blur': [1, 2, 3],
        'machines:image:blur': [1, 2],
        'machines:image:gaussian_blur': [1, 2],
        'machines:image:median_blur': [1],
        'machines:image:sharpen_hbf': [2],
        'machines:image:add_normals': [1, 3],
        'machines:image:normal_generator': [2, 3, 4],
        'machines:image:sharpen': [1, 2, 3],
        'machines:image:canny_edge_detection': [1, 2],
        'machines:image:caption': [2],
        'machines:image:create_border': [2],
        'machines:image:create_color_gray': [0, 1, 2],
        'machines:image:create_color_rgb': [0, 1, 2, 3, 4],
        'machines:image:create_color_rgba': [0, 1, 2, 3, 4, 5],
        'machines:image:create_edges': [2, 3, 4, 5],
        'machines:image:rotate': [1],
        'machines:image:shift': [1, 2],
        'machines:utility:math': [0, 2],
        'machines:utility:text_padding': [1],
        'machines:pytorch:interpolate_models': [2],
        'machines:pytorch:upscale_face': [3],
        'machines:ncnn:interpolate_models': [2],
        'machines:onnx:interpolate_models': [2],
    };
...
const gammaCheckbox: ModernMigration = (data) => {
    data.nodes.forEach((node) => {
        if (node.data.schemaId === 'machines:image:gamma') {
...
const deriveSeed: ModernMigration = (data) => {
    ...
    data.nodes.forEach((node) => {
        if (node.data.schemaId === 'machines:utility:random_number') {
            const sourceEdge = data.edges.find((e) => e.targetHandle === `${node.id}-3`);
            if (sourceEdge) {
                const id = deriveUniqueId(node.id);
                newNodes.push({
                    data: {
                        schemaId: 'machines:utility:derive_seed' as SchemaId,
...
const seedInput: ModernMigration = (data) => {
    const changedInputs: Record<string, number> = {
        'machines:external_stable_diffusion:img2img': 4,
        'machines:external_stable_diffusion:img2img_inpainting': 5,
        'machines:external_stable_diffusion:img2img_outpainting': 4,
        'machines:external_stable_diffusion:txt2img': 2,
        'machines:image:create_noise': 2,
        'machines:image:add_noise': 4,
    };
