...
export interface MachinesDragData {
    schemaId: SchemaId;
    offsetX?: number;
    offsetY?: number;
}
...
export const enum TransferTypes {
    MachinesSchema = 'application/machines/schema',
    Preset = 'application/machines/preset',
}
...
const machinesSchemaProcessor: DataTransferProcessor = (
    dataTransfer,
    { getNodePosition, createNode, schemata }
) => {
    if (!dataTransfer.getData(TransferTypes.MachinesSchema)) return false;

    const { schemaId, offsetX, offsetY } = JSON.parse(
        dataTransfer.getData(TransferTypes.MachinesSchema)
    ) as MachinesDragData;
...
const machinesPresetProcessor: DataTransferProcessor = (
    dataTransfer,
...
const openNMStudioFileProcessor: DataTransferProcessor = (dataTransfer) => {
    if (dataTransfer.files.length === 1) {
        const [file] = dataTransfer.files;
...
export const dataTransferProcessors: readonly DataTransferProcessor[] = [
    machinesSchemaProcessor,
    machinesPresetProcessor,
    openNMStudioFileProcessor,
    openFileProcessor,
];
