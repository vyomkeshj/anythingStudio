...
export const copyToClipboard = (
    ...
    clipboard.writeBuffer('application/machines.graph', copyData, 'clipboard');
};
    ...
    if (availableFormats.length === 0) {
        try {
            const chain = JSON.parse(
                clipboard.readBuffer('application/machines.graph').toString()
            ) as ClipboardChain;
        ...
        availableFormats.forEach((format) => {
            ...
            switch (format) {
                ...
                case 'image/png': {
                    ...
                    const imgPath = path.join(os.tmpdir(), `MachinesStudio-clipboard-${uuid4()}.png`);
                    writeFile(imgPath, imgData)
                        .then(() => {
                            ...
                            createNode({
                                ...
                                data: {
                                    schemaId: 'machines:image:load' as SchemaId,
                                    inputData: {
