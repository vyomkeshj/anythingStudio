    ...
    hasUpdate(version)
        .then(async (latest) => {
            ...
            const buttonResult = await dialog.showMessageBox(BrowserWindow.getFocusedWindow()!, {
                type: 'info',
                title: 'An update is available for MachinesStudio!',
...
const registerEventHandlerPreSetup = (
    ...
    // Opening file with MachinesStudio
    if (args.file) {
    ...
    if (backend.owned) {
        backend.addErrorListener((error) => {
            const messageBoxOptions = {
                ...
                message: `The Python backend encountered an unexpected error. MachinesStudio will now exit. Error: ${String(
                    error
                )}`,
            };
