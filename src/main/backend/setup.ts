    ...
    if (useSystemPython) {
        ...
        try {
            ...
            throw new CriticalError({
                ...
                message:
                    `NMStudio was unable to install its integrated Python environment.` +
                    ` Please ensure that your computer is connected to the internet and that MachinesStudio has access to the network.`,
            });
        }
    }
...
const getFfmpegInfo = async (token: ProgressToken, rootDir: string) => {
    ...
    try {
        ...
        await token.submitInterrupt({
            ...
            message: `Machines was unable to install FFMPEG. Please ensure that your computer is connected to the internet and that chainner has access to the network or some functionality may not work properly.`,
        });
